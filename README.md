...
-  Sessions are stored in server's memory whereas Json web tokens are stateless and stored in client side instead of server's memory. Sessions don't require signatures but json web tokens have cryptographic signatures. Sessions work only in a single domain whereasas Json web tokens you cn authenticate a user across multiple location with multiple domains i.e because they are stored locally in request handler.
2. What does `bcryptjs` do to help us store passwords in a secure manner?
- Bcryptjs uses the password hashing function and uses salts to secure and store the passwords.
3. How are unit tests different from integration and end-to-end testing?
- A unit test runs some arbitrary portion of the system not the whole system whereas the integration test runs just two adjoining layers of the system and end to end test runs the entire system.
4. How does _Test Driven Development_ change the way we write applications and tests?
- Test Driven Development is process where we write tests before codes. It helps us to think of he most likely scenarios where the unit could fail. This way it will help us to avoid the duplicaion of codes as we write small amount of codes at a time in order to pass the tests.
